...
def construct_rec(tags, records, mapping_dict):
    ...
    try:
        ...
        _id = _id.text
        protein_type = tags.find(
            "{http://www.hmdb.ca}protein_type")  # get protein type
        ...
        for m in tags.findall("{http://www.hmdb.ca}metabolite_references"):
            for ref in m:
                ...
                data = {}
                data = {"_id": _id2, "pmid": None, "subject": {
                    "protein_type": protein_type.text}, "object": {}}
            ...
            for met_assc_ in met_assc.findall("{http://www.hmdb.ca}metabolite"):
                for met_assc_id in met_assc_.findall("{http://www.hmdb.ca}accession"):
                    ...
                    # else create the document
                        ...
                        data = {"_id": _id+"_%s" % ct, 'pmid': 'Unknown',
                                'subject': {}, 'object': {'accession': met_assc_id.text}}
                        ct += 1  # update the id counter

                        # add the mapping ids for this accession
                        enter_mapping_ids(mapping_dict, met_assc_id.text, data)
